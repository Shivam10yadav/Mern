    ...
    <div className="space-y-6">
      ...
      {data.length===0 ?(
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="text-sm">Click "Add Experinece" to get started</p>
        </div>
      ):(
         ...
         {data.map((experience,index)=>(
           <div key={index} className="p-4 border border-e-gray-200 rounded-lg space-y-3">
             ...
             <label className="flex items-center gap-2">
              <input type="checkbox" checked={experience.is_current|| false} onChange={(e)=>{updateExperience(index,"is_current",e.target.checked ? true:false);}}  className="rounded border-e-gray-300 text-blue-600 focus:ring-blue-500"/>
