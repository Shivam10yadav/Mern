         ...
         {showcreateresume &&(
          <form onSubmit={createresume} onClick={()=>setshowcreatereume(false)  } className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e=>e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <button disabled={isCreating} className='w-full py-2 bg-gray-600 text-white rounded hover:bg-green-700 transition-colors flex items-center justify-center gap-2'>
                {isCreating && <LoaderCircleIcon className='animate-spin size-4 text-white'/>}
